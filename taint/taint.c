...
static int php_taint_fcall_check(zend_execute_data *ex, const zend_op *opline, zend_function *fbc) /* {{{ */ {
...
static int php_taint_fcall_handler(zend_execute_data *execute_data) /* {{{ */ {
	...
	if (fbc->type == ZEND_INTERNAL_FUNCTION) {
		good = php_taint_fcall_check(call, opline, fbc);
		// I have no idea why, but printing an empty string will just fix the problem
		php_printf("");
	}
